...
export class AdminService {
  ...
  async bootstrapAdmin(email: string) {
    ...
    if (existingAdmin) {
      throw new Error("An admin user already exists. Use the admin panel to create additional admins.");
    }
    ...
    if (!user) {
      throw new NotFoundException(`User with email "${email}" not found. Please register first.`);
    }
    ...
    return this.prisma.user.update({
      ...
      data: { 
        isAdmin: true,
        is18PlusVerified: true // Auto-verify admin
      },
      select: {
        id: true,
        email: true,
